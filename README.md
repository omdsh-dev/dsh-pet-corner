...
```bash
# 已登录私有 npm registry
dsh plugin --profile web add @deepseek-ai/dsh-pet-corner@0.0.1-rc.2
...
- 设置通过插件自有的 `/plugins/dsh-pet-corner/api/settings` 读写，不依赖 DSH rc.2
...
宿主路由使用 rc.2 的 `webServer` 服务名；本版本不兼容 rc.1 的 `httpServer` 名称。
